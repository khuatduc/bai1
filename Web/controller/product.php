	...
	class product
	{
		...
		public function insert_product($date,$files){
			...
			if($product_code ='' || $productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				$alert = "<span class='error'>Cac trường không được bỏ trống</span>";
				return $alert;
			}else{
		...
		public function update_product($data,$files,$id){
			...
			if($product_code == "" || $productName=="" || $productQuantity=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type==""){
				$alert = "<span class='error'>Hình ảnh không được bỏ trống</span>";
				return $alert; 
			}else{
				if(!empty($file_name)){
					//Nếu người dùng chọn ảnh
					if ($file_size > 204800) {
		    		 ...
		    		 $alert = "<span class='success'>Kích thước ảnh <=2MB</span>";
					return $alert;
				    } 
					elseif (in_array($file_ext, $permited) === false) 
					{
		...
		public function timkiemproduct( $tukhoa){
